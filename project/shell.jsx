      ...
      <div className="loader" style={{ opacity: exiting ? 0 : 1, transition: "opacity 0.2s" }}>
        <div className="loader-inner">
          <div className="loader-row">
            <span>MODI · ENTERING SYSTEM</span>
    ...
    <>
      <div className={`detail ${enter ? "on" : ""}`}>
        <div className="detail-left">
          <div className="detail-left-art">
            <Art accent={accent} />
            <button className="detail-close" onClick={handleClose}>esc</button>
          </div>
          <div className="detail-left-foot">
            <div style={{ fontFamily: "var(--mono)", fontSize: 10, letterSpacing: "0.2em", color: "var(--fg-mute)", marginBottom: 8, textTransform: "uppercase" }}>{project.n} · {project.year}</div>
            <h2 className="detail-title">{project.title} <em>{project.em}</em></h2>
            <div style={{ display: "flex", gap: 10, marginTop: 14, flexWrap: "wrap" }}>
              <span className="detail-badge">{project.role}</span>
              <span className="detail-badge">{project.scale}</span>
            </div>
          </div>
        </div>
        <div className="detail-right">
          {project.problem && (
            <div>
              ...
              <p className="detail-problem">{project.problem}</p>
            </div>
          )}
          {project.architecture && (
            <div>
              <div className="detail-section-label">architecture</div>
              <ul className="detail-arch-list">
                {project.architecture.map((item, i) => <li key={i}>{item}</li>)}
              </ul>
            </div>
          )}
          {project.impact && (
            <div>
              ...
              <div className="detail-impact">
                {project.impact.map((m, i) => <span key={i} className="detail-impact-chip">{m}</span>)}
              </div>
            </div>
          )}
          <div>
            <div className="detail-section-label">stack</div>
            <div className="detail-stack">
              {project.tags.map((t) => <span key={t} className="project-tag">{t}</span>)}
            </div>
          </div>
          {project.link && (
            <a href={project.link} target="_blank" rel="noopener noreferrer" className="detail-gh-link">
